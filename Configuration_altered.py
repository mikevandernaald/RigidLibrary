...
class Configuration:
        ...
        def __init__(self,datatype,N,radii,mu0=0.2,prefix10='DSC',prefix20='_solved_Tracke_'):
            #self.folder = folder
            self.datatype=datatype
            self.addBoundary=False
            if (datatype=='simulation'):
                print ("Reading simulation data!")
                self.getParameters(N,1,1,1,1,1,1,1,radii)
        ...
        def getParameters(self,N,krep,Lx,Ly,mu,xi,phi,strain_rate,radii):
            self.N = N
            self.krep = krep
            self.Lx = Lx
            self.Ly = Ly
        ...
        def readSimdata(self,contactData,distSnapshot0=100.0):
            self.noConf = False
            ...
            self.strain=self.gammadot*(1.0*1)*self.distSnapshot
